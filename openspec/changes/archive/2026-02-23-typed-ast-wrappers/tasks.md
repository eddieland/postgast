...
- For each of the 276+ message types, classifies every field by kind:
...
- Generates `_REGISTRY.update()` at the bottom of `_generated.py`
- Handles edge cases: `A_Const` (which has its own `val` oneof), `List`/`IntList`/`OidList` wrapper types

The script writes only two files into `src/postgast/nodes/`:

- `_generated.py` — all wrapper classes + `_REGISTRY.update()`
- `__init__.py` — re-exports `AstNode`, `wrap`, and all wrapper class names

The script does **not** generate `base.py`. The `AstNode` base class, `_REGISTRY`, `wrap()`, `_wrap_node_optional()`,
and `_wrap_list()` helpers are hand-written in `base.py`.

**Files:** `scripts/generate_nodes.py` (new), `src/postgast/nodes/base.py` (new, hand-written)

**Acceptance criteria:**

- Script runs successfully: `uv run python scripts/generate_nodes.py`

- Output files pass `ruff check` and `ruff format`

- Output files pass `basedpyright` type checking

- All 276+ message types have corresponding wrapper classes

- [x] ### 2. Generate the initial nodes package

Run the generation script and commit the output.

**Files:** `src/postgast/nodes/_generated.py` (new, generated), `src/postgast/nodes/__init__.py` (new, generated)

**Acceptance criteria:**

- Files are syntactically valid Python

- `_generated.py` imports only from `postgast.nodes.base` and (under `TYPE_CHECKING`) `postgast.pg_query_pb2`

- Every class in `pg_query_pb2.pyi` that extends `_message.Message` has a corresponding wrapper class

- `_REGISTRY.update()` has an entry for each wrapper class
