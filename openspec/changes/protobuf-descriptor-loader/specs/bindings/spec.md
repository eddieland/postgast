...
Surface equivalence SHALL be understood behaviourally, not only by name: published message classes SHALL carry a
`__module__` naming the module they are published from, so that messages remain picklable as they are under gencode.

The module SHALL locate its descriptor set by filesystem path relative to itself, and SHALL NOT import the `postgast`
package to do so, so that it remains loadable in isolation before the native library is available.
...
#### Scenario: Messages remain picklable

- **WHEN** a message built by the module is passed to `pickle.dumps` and back through `pickle.loads`
- **THEN** it round-trips with identical field values

#### Scenario: Loadable without the native library

- **WHEN** the module is loaded directly by file path, before the libpg_query shared library has been built
- **THEN** it imports successfully and `DESCRIPTOR` is populated, so `scripts/generate_nodes.py` can walk it

#### Scenario: Descriptor is available on every protobuf backend

- **WHEN** the module is imported under either the upb or the pure-Python protobuf implementation
- **THEN** `DESCRIPTOR` is a `FileDescriptor` rather than `None`
...
- **THEN** `pg_query.desc` is present alongside the `postgast` package modules
