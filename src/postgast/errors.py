...
class PgQueryError(Exception):
    ...
    Every postgast function that calls into libpg_query (:func:`~postgast.parse`, :func:`~postgast.deparse`,
    :func:`~postgast.normalize`, :func:`~postgast.fingerprint`, :func:`~postgast.split`, :func:`~postgast.scan`,
    :func:`~postgast.parse_plpgsql`, and :func:`~postgast.format_sql`) may raise this exception. The error carries the
    ...
    Attributes:
        ...
        context: Additional context from the parser (e.g., PL/pgSQL function name), or ``None``.
